...
In single-user mode, the system SHALL read `AUTH_METHOD` at startup. Accepted values: `none` (default), `password`, `mtls`, `gitlab`.
...
#### Scenario: AUTH_METHOD=mtls requires a valid client certificate
- **WHEN** `AUTH_METHOD=mtls` and a request arrives without a valid client TLS certificate
- **THEN** the TLS handshake is rejected before the HTTP handler runs

#### Scenario: AUTH_METHOD=mtls requires TLS to be enabled
- **WHEN** `AUTH_METHOD=mtls` and TLS is not configured (no cert/key)
